...
| `caffeine.sh` | Toggle `caffeinate -disu` + `pmset disablesleep`; watches the lid and locks the screen on close (macOS fires no sleep event when sleep is disabled, so its own "lock immediately" rule never runs) | `prefix + C`. State is read from `pmset SleepDisabled`, surfaced as `@caffeine` in the left status bar. `--selftest` checks the password reader. |
